...
\begin{description}[style=nextline]
  \item[\req{REQ-R-01}] The roster shall be the union of the compiled-in
        \code{SWIMMERS} table and the \code{swimmer\_alias} table of the
        shared database.  A swimmer admitted to that database by
        \code{addswimuser} shall be selectable by this program on the
        next run, with no rebuild.
  \item[\req{REQ-R-02}] Where an alias appears in both, the compiled-in
        entry shall win.  Its \code{match\_substr} has been tuned by
        hand against real search results, and behaviour shall not change
        merely because a row was added elsewhere.
  \item[\req{REQ-R-03}] A database that cannot be read shall be a
        warning, not a failure, for every mode except \code{offline}:
        the compiled-in roster still works, and refusing to fetch a
        known swimmer because a remote server is down is a poor trade.
  \item[\req{REQ-R-04}] The program shall provide \code{-o list},
        printing every selectable alias with the swimmer it names, its
        \code{memberId} if known, and whether it came from the binary or
        the database.  The roster is no longer legible by reading the
        source, so an operator needs some way to see what the program
        will accept.  \code{-o list} shall require the database but not
        the network, and shall not sign in.
...
\begin{longtable}{ll}
  ...
  REQ-R-01 \ldots REQ-R-04 & Test (merged roster, \texttt{-o list}) \\
...
\vfill
\begin{center}
  \small\textit{End of document CAST-SRS-001 Rev.\,7.0}
\end{center}
