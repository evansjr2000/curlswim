...
canonical source is \code{swim2/swim-times.w}.
...
selected swimmer it queries the live USA~Swimming data-hub REST
services, retrieves the best recorded time for each event (with its
date, meet, and motivational standard), sorts them fastest-first, and
prints either a human-readable table or a CSV stream suitable for
...
\begin{enumerate}[label={[\arabic*]}]
  ...
  \item USA~Swimming, \textit{Data Hub Times REST API (OpenAPI)},
        \texttt{https://times-api.usaswimming.org/swagger/v1/swagger.json}.
        Accessed 2026.

  \item \code{swim2/swim-times.w} --- CWEB literate program source
...
\begin{enumerate}
  \item Live data is fetched directly from the authoritative
        USA~Swimming data-hub REST services, removing the human
...
the CWEB literate program \code{swim-times.w}.  It requires no
daemon and no network service beyond HTTPS access to
\texttt{times-api.usaswimming.org}.  An optional local Postgres
database (\texttt{swim-times}) may be used to persist results and
to serve subsequent queries offline.
...
\begin{description}[style=nextline]
  \item[Swimmer selection (\code{-o} flag).]
        ...
        \code{ledecky10}, \code{ledecky12}, \code{ledecky14}) to
        restrict output.  When no swimmer keyword is given all
        seven \texttt{SWIMMERS[]} entries are processed (the four
        Evans/Benavente swimmers plus three age-windowed Ledecky
        entries that share the same person).  \code{ledecky10}
        retrieves Katie Ledecky's swims as a 9- and 10-year-old
        (17~March~2006--16~March~2008); \code{ledecky12} the 11-
        and 12-year-old window (17~March~2008--16~March~2010);
        \code{ledecky14} the 13- and 14-year-old window
  ...
  \item[Database persistence (\code{store} keyword).]
        Each retrieved row is also written to the local Postgres
        database \texttt{swim-times} programmatically via the
        \texttt{libpq} client library --- one prepared
        \texttt{INSERT \dots\ ON~CONFLICT~DO~NOTHING} per row,
        keyed on \texttt{(swimmer, event, swim\_time, swim\_date,
        meet)}.  Re-storing already-present rows is a silent
        no-op rather than an error.  Combine with \code{csv} to
        also echo the rows to standard output.

  \item[Offline retrieval (\code{offline} keyword).]
        All network calls are skipped; rows are read from the local
        Postgres \texttt{swim-times} database (populated previously
        by \code{store}) and printed in the same table or CSV form.

  \item[Live authoritative data.]
        Online runs fetch from the USA~Swimming data-hub REST
        services (member search plus the anonymous
        \code{BestTimes} query); no local cache is consulted unless
        \code{offline} is selected.
...
\begin{itemize}
  ...
  \item \code{libcurl} with TLS support (for HTTPS to the
        USA~Swimming data-hub REST services).
  ...
  \item \texttt{libpq} (the Postgres C client library) at link and
        run time when the \code{store} or \code{offline} keywords
        are used.  On macOS this ships with the Homebrew
        \texttt{libpq} formula; the project's \texttt{Makefile}
        autodetects \texttt{pg\_config} either on \texttt{\$PATH}
        or at \texttt{/opt/homebrew/opt/libpq/bin/pg\_config}.  No
        host \texttt{psql} client is required.  The local Postgres
        container is still started by \texttt{pg.sh start} and the
        \texttt{swim-times} database is created by applying
        \texttt{schema.sql}.
\end{itemize}
...
\begin{description}[style=nextline]
  ...
  \item[Network.]
        ...
        Skipped under \code{offline}.

  \item[Database.]
        Optional local Postgres reached \emph{programmatically}
        via the \texttt{libpq} client library --- no
        \texttt{psql} child process and no shell wrapper.
        \code{store} runs a prepared
        \texttt{INSERT \dots\ ON~CONFLICT~DO~NOTHING} per row;
        \code{offline} runs a parameterised
        \texttt{SELECT \dots\ FROM swim\_times}.  Connection
        parameters come from the
        \texttt{SWIM\_TIMES\_PGCONNINFO} environment variable
        when set, otherwise the default keyword string
        \texttt{dbname=swim-times user=postgres host=localhost
        port=5432}.
...
\begin{enumerate}
  ...
  \item Coach runs:
        ...
        \end{center}
  \item swim-times resolves each swimmer's \code{memberId} via the
        USA~Swimming member search.
...
\begin{enumerate}
  \item Coach runs:
        ...
        \end{center}
  \item swim-times resolves Kalea's \code{memberId}, queries the
...
\subsection{Scenario 5 --- Cache-and-replay (store then offline)}

\textit{Context:} A team analyst wants a snapshot of all
swimmers' SCY times that they can re-query repeatedly without
hitting the USA~Swimming API.

\begin{enumerate}
  \item Analyst starts the local Postgres container and applies
        the schema once:
        \begin{center}
          \code{../../pg/pg.sh start} \hskip 1em
          \code{./test-db.sh}
        \end{center}
  \item Analyst captures live data into the database:
        \begin{center}
          \code{./swim-times -o stella,csv,store}
        \end{center}
        Each row is both printed and inserted into
        \texttt{swim\_times}.
  \item Later, the analyst replays the snapshot offline:
        \begin{center}
          \code{./swim-times -o stella,offline,fastest}
        \end{center}
        No HTTP request is made; rows are read from Postgres and
        printed in the same form as an online run.
\end{enumerate}

\subsection{Scenario 6 --- Historical comparison (Ledecky 9--10)}

\textit{Context:} A parent wants to compare Stella's 9-and-under
SCY times with Katie Ledecky's swims at the same age.

\begin{enumerate}
  \item Parent runs:
        \begin{center}
          \code{./swim-times -o ledecky10,fastest -e "100 FR SCY"}
        \end{center}
  \item swim-times resolves Katie Ledecky's \code{memberId} and
        prints her lifetime-best 100~FR~SCY time with its swim date
        (the anonymous \code{BestTimes} query returns one best per
        course; the recorded date lets the parent see the age at
        which it was set).
  \item Parent compares against Stella's current best obtained by
        \code{./swim-times -o stella,fastest -e "100 FR SCY"}.
\end{enumerate}
...
No remote infrastructure is required.  An optional local Postgres
instance (run as a Docker container managed by the
\texttt{pg.sh} wrapper script) provides persistence and offline
replay; absence of that container simply disables the
\code{store} and \code{offline} keywords without affecting any
other capability.  The binary carries no embedded credential; it
authenticates anonymously with run-time-generated data-hub headers,
so no token rotation is ever required.  A recompile is needed only if
USA~Swimming changes its REST endpoint URLs or request shapes.  These
are maintenance events, not operational events.

\subsection{Security}

No credential is embedded or stored.  The data-hub REST services
serve only public competitive records and accept anonymous callers
that supply three headers --- \code{Usas-Sub-Id: Anonymous},
\code{AppName: DataHub}, and a client-minted \code{Device-Id} whose
format (not registration) the server validates.  \code{swim-times}
generates a conforming \code{Device-Id} at run time, so there is no
static secret to leak or rotate and no runtime credential injection is
needed.
...
\begin{description}[style=nextline]
  ...
  \item[Full JSON parsing library (e.g., \code{cJSON}, \code{jansson}).]
        ...
        REST response format is stable and well-understood,
...
artefact (the typeset \code{swim-times.pdf}) that serves
