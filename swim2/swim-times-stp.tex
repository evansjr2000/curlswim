...
\begin{titlepage}
  ...
  \begin{tabular}{ll}
    ...
    Status:              & Released                                           \\
    Supersedes:          & Revision 6.0, September 19, 2026                    \\
...
CAST-STP-001, revision 7.0.  This revision supersedes revision 6.0 in
its entirety.
...
\subsection{Rationale for revision 7.0}

The roster is no longer compiled in.  It is now the union of the
\code{SWIMMERS} table in \code{swim-times.w} and the
\code{swimmer\_alias} table of the shared database, so a swimmer
admitted by the sibling \code{addswimuser} program is selectable here on
the next run without a rebuild.  That closes the drift this plan has
noted since revision 5.0.

It also changes what a test can assume.  The set of valid \code{-o}
tokens is no longer knowable from the source, so \req{REQ-R-02} derives
its expectation from the database itself --- it reads
\code{swimmer\_alias} and requires every alias there to appear in
\code{-o list}.  A case that hard-coded the roster would pass while the
feature silently regressed.

\req{REQ-R-03} guards the collision rule in the direction that matters:
\code{ledecky10} exists in both, and the compiled-in entry must win, so
that adding a row elsewhere cannot quietly change how a tuned lookup
behaves.
...
\begin{longtable}{p{2.1cm}p{5.2cm}p{6.2cm}}
  ...
  \textbf{ID} & \textbf{Requirement} & \textbf{Method} \\
  \midrule\endhead
  \req{REQ-R-01} & \code{-o list} prints the merged roster
    & At least the compiled-in count, and no sign-in: the listing needs
      the database, not the network. \\
  \req{REQ-R-02} & Every database alias is selectable
    & The expectation is read from \code{swimmer\_alias} at run time,
      not hard-coded: every alias the table holds must appear in
      \code{-o list}.  This is the case that would catch the feature
      regressing. \\
  \req{REQ-R-03} & Compiled-in entries win a collision
    & All 29 remain, and \code{ledecky10} --- which exists in both ---
      is reported as \code{compiled}. \\
  \req{REQ-R-04} & A database-only alias resolves and fetches
    & The first alias marked \code{database} is fetched for one event
      and must produce a \code{Swimmer:} line.  Skipped when not signed
      in. \\
  \bottomrule
\end{longtable}
\end{center}
...
\begin{longtable}{p{4.0cm}p{1.5cm}p{1.5cm}p{6cm}}
  ...
  The shared database's schema moves
      & Med  & High & \req{DB-04} through \req{DB-11} check the shape
                      ...
                      name what changed. \\
  \code{swimmer\_alias} gains a malformed row
      & Med  & Low  & A row missing any of alias, query, or substring is
                      skipped rather than half-used; \req{REQ-R-02}
                      reports it as an alias absent from the roster. \\
  The two rosters disagree
      & Med  & Low  & \req{REQ-R-03} fixes the precedence and asserts
                      it.  \code{-o list} shows the provenance of every
                      alias, so a disagreement is visible rather than
                      inferred. \\
