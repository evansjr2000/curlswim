...
verification of upstream USA~Swimming data, the data-hub REST
back-end,
...
swim times from the USA~Swimming data hub via its public REST API.
...
\begin{itemize}
  \item The test host has outbound HTTPS access to
        \texttt{times-api.usaswimming.org}.
  \item The anonymous data-hub REST services remain reachable
        (member search and \code{BestTimes}) on the day the test is
        executed; no credential is required.
  \item The four tracked swimmers continue to exist in the
        USA~Swimming member directory.
...
\begin{longtable}{p{4.0cm}p{1.5cm}p{1.5cm}p{6cm}}
  ...
  \textbf{Risk} & \textbf{Likeli-} & \textbf{Impact} & \textbf{Mitigation} \\
                & \textbf{hood}     &                  & \\
  \midrule\endhead
  Device-Id format rejected by server
            & Low  & High & \code{device\_id()} updated to match the
                            current data-hub format; recompile and
                            re-run STP. \\
  REST response schema change
...
Testing is decomposed into five sub-processes, executed in order
...
\begin{description}[style=nextline]
  ...
  \item[Online behaviour sub-process.]
        ...
        REST requests against the data-hub times service.  Skipped
        cleanly when network is unreachable or the operator sets
        \code{SWIM\_TIMES\_SKIP\_NETWORK=1}.
  \item[Database persistence sub-process.]
        \req{REQ-F-70} \dots\ \req{REQ-F-75} exercise the
        \code{store} write path and the \code{offline} read path
        against a local Postgres container.  The application
        binary reaches the database programmatically through the
        \texttt{libpq} client library (\req{REQ-I-12}) using a
        connection string taken from
        \texttt{SWIM\_TIMES\_PGCONNINFO} or the default
        \texttt{dbname=swim-times user=postgres host=localhost
        port=5432}.  The companion bash helper \code{test-db.sh}
        starts the container via \texttt{pg.sh}, applies
        \texttt{schema.sql}, and prints a summary of
        \texttt{swim\_times}; bash test-driver scripts may
        continue to use \texttt{pg.sh psql} for setup/teardown
        because that constraint applies only to the application
        binary, not to test infrastructure.  Tests are skipped
        cleanly when the database cannot be reached.
\end{description}
...
\begin{itemize}
  ...
  \item Optional Postgres container managed by the project's
        \texttt{pg.sh} wrapper script (Docker required); only
        the database persistence tests depend on it.  In its
        absence those tests skip and the run still gates on the
        remaining suites.
\end{itemize}

\subsection{Test data}

The five production swimmer records and the thirty-one
production event codes serve as test data for the online
suites.  The database persistence suite uses whatever rows the
\code{store} sub-test inserts in the same run; no fixture file
is checked in.  No synthetic REST data is required because
the system under test is read-only against a live read-only API.
...
\begin{itemize}
  ...
  \item More than three consecutive online tests fail with
        identical HTTP error codes (suggests a data-hub-side
...
\begin{tabular}{ll}
  ...
  Data-hub \code{Device-Id} format changes       & Run STP after recompile          \\
...
\begin{longtable}{p{2.6cm}p{1.0cm}p{8.4cm}}
  ...
  \req{REQ-F-11}    & D & \code{-e ...} with no swimmer keyword yields seven \code{Swimmer:} lines \\
  \req{REQ-F-12}    & D & \code{stella} keyword resolves to a row containing ``Evans'' \\
  \req{REQ-F-13}    & D & \code{ledecky10}/\code{ledecky12}/\code{ledecky14} resolve to a row containing ``Katie Genevieve Ledecky'' \\
  \req{REQ-F-14}    & I & \code{ledecky10} window [2006-03-17, 2008-03-16] present on the \code{Swimmer} entry; applied to the offline \texttt{SELECT} \\
  \req{REQ-F-15}    & I & \code{ledecky12} window [2008-03-17, 2010-03-16] present on the \code{Swimmer} entry \\
  \req{REQ-F-16}    & I & \code{ledecky14} window [2010-03-17, 2012-03-16] present on the \code{Swimmer} entry \\
  \req{REQ-F-16a}   & T & Online \code{ledecky10} run reports a best time with a non-empty date \\
  ...
  \req{REQ-F-32}    & T & Table row carries a non-empty date, standard, and meet for a known swim \\
  \req{REQ-F-33}    & A & One \code{BestTimes} POST per stroke-and-distance pair verified by source inspection \\
  ...
  \req{REQ-F-60}    & I & Lookup-failure path inspected in \code{lookup\_member\_id} \\
  \req{REQ-F-61}    & I & Per-event failure-then-continue inspected in \code{fetch\_times} \\
  \req{REQ-F-70}    & T & \code{store,csv} run inserts $\geq$1 row into \texttt{swim\_times} and prints CSV (DB-gated) \\
  \req{REQ-F-71}    & T & \code{offline,csv} run reads $\geq$1 previously stored row (DB-gated) \\
  \req{REQ-F-72}    & T & \code{offline} run completes with \texttt{SWIM\_TIMES\_SKIP\_NETWORK=1} (DB-gated) \\
  \req{REQ-F-73}    & T & Repeated \code{store} run leaves \texttt{count(*)} unchanged (DB-gated) \\
  \req{REQ-F-74}    & I & Missing-\texttt{libpq}-connection warning path inspected in \code{db\_open} \\
  \req{REQ-F-75}    & T & Re-\code{store} run exits 0 with no stderr (ON CONFLICT DO NOTHING) (DB-gated) \\
  ...
  \req{REQ-I-12}    & I & \code{db\_conn\_string} reads \texttt{SWIM\_TIMES\_PGCONNINFO}; binary linkage includes \texttt{libpq} \\
  \req{REQ-I-20}    & A & \code{https://usaswimming.sisense.com} substring present, no \code{http://} \\
  \req{REQ-P-01}    & D & Sample timing extrapolated to 160 requests $<$ 300 s        \\
  \req{REQ-P-02}    & D & Single query measured $<$ 10 s                              \\
  \req{REQ-P-03}    & D & Offline single query measured $<$ 2 s (DB-gated, manual)    \\
  ...
  \req{REQ-D-04}    & A & Binary dependency list contains only \{libc, libcurl, libpq, TLS, support libs\} \\
  \req{REQ-D-10}    & I & \texttt{schema.sql} declares \texttt{swim\_times} with the UNIQUE constraint per \req{REQ-F-73} \\
